...
namespace sensor{
    ...
    bool TimeOfFlight::set_long_range() {
        // need equivalent from arduino libraries of Vcselperiod Pre and Final Range. these are from polulu driver.
    ...
    bool TimeOfFlight::set_short_range() {
        //VL53L0X.setVcselPulsePeriod(VL53L0X::VcselPeriodPreRange, 14);
        //VL53L0X.setVcselPulsePeriod(VL53L0X::VcselPeriodFinalRange, 10);
    ...
    void TimeOfFlight::set_schedule_period(int period) { // milliseconds/10
        ...
        VL53L0X.setMeasurementTimingBudgetMicroSeconds(period * 1000); // microseconds
    }
    ...
    bool TimeOfFlight::set_address(int new_address) {
        if(!VL53L0X.setAddress(new_address))
            return TOF_NOT_OK;
        return TOF_OK;
    }
}
